...
def post_fork(server, worker):
    ...
    try:
        ...
        s.bind(("0.0.0.0", port))  # Bind to the correct port
        print(f"Socket test successful, port: {s.getsockname()[1]}")
        s.close()
...
try:
    ...
    test_socket.bind(("0.0.0.0", port))  # Bind to the specified port
    print(f"Pre-startup socket test successful on port: {test_socket.getsockname()[1]}")
    test_socket.close()
except Exception as e:
